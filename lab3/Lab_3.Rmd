...
```{r hypothesis_testing_data_files, include=FALSE, eval=FALSE}
t_test_sex_ps_file <- '../data/GSE112179_t_test_sex_ps.rgcs'
aov_diagnoses_ps_file <- '../data/GSE112179_aov_diagnoses_ps.rgcs'
```
...
saveRDS(t_test_sex_ps, file=t_test_sex_ps_file)
```

```{r t_test_sex_ps_load, include=FALSE, eval=FALSE}
t_test_sex_ps <- readRDS(file=t_test_sex_ps_file)
```
...
saveRDS(chr7_position_idx, file='../data/GSE112179_chr7_position_idx.rgcs')
```
...
```{r chr7_sex_effect_sizes, include=FALSE, eval=FALSE}
# effect_size = (mean1 - mean2) / stdev
chr7_male_mean
```
...
# Methylation values of the male samples for 7th chromosome positions
mean(beta[chr7_position_idx,male_beta_idx])

# Methylation values of the female samples for 7th chromosome positions
mean(beta[chr7_position_idx,female_beta_idx])

effect_size <- abs(mean(beta[chr7_position_idx,male_beta_idx]) - mean(beta[chr7_position_idx,female_beta_idx]))
effect_size
effect_size/sd(beta[chr7_position_idx,])

#volcanoPlot()

# Should volcano plot be one that contains all chromosomes within it?
# Or should there be a volcano plot for each chromosome?

# How effect size needs to calculated? There should be an effect size for each 
# position?
```

```{r hypothesis_testing_diagnoses, include=FALSE, eval=FALSE}
# Tests will be performed for each row (position) in the data set
position_num <- length(beta[,1])
...
for(i in 1:position_num) {
  print(i)
  aov_diagnoses_ps <- append(aov_diagnoses_ps, 
                             unlist(summary(aov(beta[i,] ~ diagnoses)))[9])
}

length(aov_diagnoses_ps)

```

```{r aov_diagnoses_ps_save, include=FALSE, eval=FALSE}
saveRDS(aov_diagnoses_ps, file='../data/GSE112179_aov_diagnoses_ps.rgcs')
```
...
After all gene name manipulations there were selected 10686 foreground positions
and 27304 background positions.

Go result is crazy (see GO.png). So... to be continued...
# References {-}
