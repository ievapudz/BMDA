...
For the third task it was required to perform hypothesis testing for each 
position for samples grouped in several different ways. In our case, we decided 
to define these groups based on: sex, race, and diagnosis type.
...
#races <- sample_annot@colData$race
```
...
higher than \(\alpha = 0.05\), therefore it could be stated that the
...
It was required to draw volcano plots for each chromosome's effect sizes and 
p-values. Since collection of beta matrix's indeces of positions that belong to 
a particular chromosome is a time-consuming process, it was decided to plot one
volcano plot for the 7th chromosome at first, and if there is enough time left,
repeat the flow for other chromosomes
...
volcano_plot <- ggplot(effect_sizes_p_values, aes_(x=effect_sizes_p_values$x, 
  y=effect_sizes_p_values$y)) + geom_point() + 
  geom_pointdensity() +
  scale_color_viridis() +
...
sizes of the top 5 most significant methylation values are: 
...
Since values are negative they all are represented in the upper left part of the
volcano plot.
...
foreground <- c(noquote(unique(gsub(";", " ",pos_annot$UCSC_RefGene_Name[indexes]))))
...
After all genes names manipulations there were selected 10686 foreground genes
with FDR correction and 27304 background genes.

For gene ontology enrichmet analysis on statistically significant positions
we used GOrilla tool. 

In components part (Figure 20),
the highest enrichment is found in cellular components such as nucleus, axon 
and neuron parts, synaptic membrane. These results reflect associations with 
the isolated neurons from prefrontal cortex samples. 
...
In terms of processes (Figure 19), the highest enrichment is found
...
are one of the main indicators of data quality as they relate to the above mentioned
components. In metabolic processes it would be interesting to make
further investigation on regulation of kinase activity, transferase activity and 
phosphate metabolic processes as there are only separate studies and traces of
real possible intersections. Also fascinating outcomes were found examining
...
morphogenesis compared to healthy patients, so it indicates correlation with
developmental processes.

\begin{figure}[!h]
  \begin{center}
    \includegraphics[width=150mm]{./go_process_part_II.png}
    \caption{
...
Highest enrichment in functions (Figure 21) is found in binding and catalytic 
activity. (But why tho?)

\begin{figure}[!h]
  \begin{center}
    \includegraphics[width=150mm]{./go_function_part.png}
    \caption{
...
based on age (groups of sexes and diagnoses). 
...
\begin{figure}[!h]
  \begin{center}
    \includegraphics[width=150mm]{./aov_age_hist.png}
    \caption{
    The histogram of p-values of ANOVA test that compared sample sex and diagnosis
    groups with and without age group.
    }
  \end{center}
\end{figure}
...
These positions belong to chromosome 6 (cg27026819 and cg05191364), 19 
(cg02426178), 16 (cg06220235), and 10 (cg18514820).

## Manhattan plot

Because the Manhattan plot in sex-based hypothesis testing was used on
7 chromosome, for the more detailed comparison, in the age-based test 
we also plotted Manhattan plot (Figure 24) with p-values of positions in 
this chromosome.
...
As Manhattan plot, Volcano plot was repeated on 7th chromosome for further
comparison.
...
The volcano plot (Figure 25) shows 4 outliers that are downregulated in samples with age
younger than 47 years old and 1 outlier in samples with age older than 47.
Effect sizes of the top 5 most significant methylation values are: 
...
\begin{figure}[!h]
  \begin{center}
    ...
    \caption{
     Volcano plot of all positions effect sizes and p-values of ANOVA test for data 
     grouped to people younger than 47 years and older than 47 years. 
    }
  \end{center}
\end{figure}
