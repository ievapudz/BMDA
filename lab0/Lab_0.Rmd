...
products accordingly. The information about the products that cause the most 
serious symptoms was requested. More formally, answers to the following 
...
described in the separate section.
...
Consequentially, the dataset which was used for analysis consisted of 10 columns 
...
After surfing the data, there were cases of death reported, which would 
naturally be the most harmful symptom to one's health. However, the analysis to
reported cases of death will be presented below, thus for this section 
an attempt of filtering the most serious symptoms was attempted. 

Seriousness can be interpreted differently. In our view, any form of cancer,
stroke, or paralysis are serious symptoms, or, more precisely, outcomes. 
Although, these groups are rather diverse.

There were:
- 9 different types of death reports
- 70 different types of cancer reports
- 5 different types of stroke reports
- 15 different types of reports related to brain
- 8 different types of reports related to heart
- 4 different types of paralysis reports 
...
Overall, there were `r length(unique(lethal_reports$outcome))` different 
outcomes that included death as one of them.
...
No, there was only one report regarding 
`r lethal_reports[lethal_reports$type == "concomitant", c("product")]` product.
...
shown. For more clarity, "treemap" package was used to visualise proportions of 
products that are suspected to cause death, since the most 
common argument against pie charts is that varied angles complicate the process 
of detecting differences between areas [4].
