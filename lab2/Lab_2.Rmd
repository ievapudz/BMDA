...
colnames(beta) <- dendrogram_names
...
rownames(top_N_beta) <- dendrogram_names
colnames(top_N_beta) <- dendrogram_names
...
years. Horvath's clock was trained on samples from various tissues, thus it
is a universal choice for any kind of samples. Hannum's clock was trained
on blood samples, nonetheless the tissue type does not match neural one, which
was taken to collect our data set, blood is often taken as an indicator of
the state of the whole organism. The same considerations can be applied to 
support the choice of Levine's clock. 
...
are no differences in predicted biological age between control, bipolar, and 
