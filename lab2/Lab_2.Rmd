...
#dyn_clustered <- cutreeDynamic(dendro=clustered, method='tree')
#table(dyn_clustered)
#colors <- labels2colors(dyn_clustered, colorSeq=c('#666666','#333333'))
...
cluster_3_indeces <- clustered$order[53:97]
```
...
```{r include=FALSE, eval=FALSE}
cluster_tab <- matrix(c(dg_control, dg_bipolar, dg_schizo, 
                        sum(dg_control, dg_bipolar, dg_schizo),
                        mg_control, mg_bipolar, mg_schizo, 
                        sum(mg_control, mg_bipolar, mg_schizo),
                        lg_control, lg_bipolar, lg_schizo, 
                        sum(lg_control, lg_bipolar, lg_schizo)), ncol=4, 
                        byrow=TRUE)
colnames(cluster_tab) <- c('Control','Bipolar','Schizophrenia', 'Overall')
rownames(cluster_tab) <- c('Dark gray','Medium gray','Light gray')
cluster_tab <- as.table(cluster_tab)
```
