...
rownames(dist_m) <- colnames(beta)
colnames(dist_m) <- colnames(beta)
#rownames(dist_m) <- sample_annot@colData$diagnosis
#colnames(dist_m) <- sample_annot@colData$diagnosis
dist_m
```
...
for(i in 1:length(sample_annot@colData$diagnosis)) {
  if(sample_annot@colData$diagnosis[i] == 'control') {
    # control - light gray
    colors <- append(colors, '#E0EEEE')
  }
  if(sample_annot@colData$diagnosis[i] == 'bipolar') {
    # bipolar - medium gray
    colors <- append(colors, '#C1CDCD')
  }
  if(sample_annot@colData$diagnosis[i] == 'schizophrenia') {
    # schizophrenia - dark gray
    colors <- append(colors, '#838B8B')
  }
}

plotDendroAndColors(clustered, colors, 
...
The groups are denoted by colours: dark, medium, and light gray. 
